(
	function ( $ ) {
		let session_cookie = Cookies.get("sme_session");
		const home_logo = $('.home-logo>a');
		let animation = bodymovin.loadAnimation( {
			container: home_logo[0],
			renderer: 'svg',
			loop: false,
			autoplay: session_cookie ? false : true,
			path: 'https://www.sonymusic.com/wp-content/themes/sonymusic/images/new/SM-LOGO-VERT-RED-WHITE.json'
		} );	

		  if (session_cookie !== "1") {
			Cookies.set("sme_session", "1");
		  } else {
			animation.goToAndStop(148, true);
		  }
		...
		$( window ).load( function () {
			...
			function adjustStyle( viewPort ) {
				...
				if ( viewPort >= 320 && viewPort <= 480 ) {
					...
					main_slider = $( '.slide-inner' ).bxSlider( {
						mode: 'fade',
						slideWidth: 600,
						minSlides: 1,
						maxSlides: 1,
			...
			if ( window.location.href.indexOf( '?open=true&thankyou' ) > - 1 ) {
				$.fancybox.open( [{ href: '.th', type: 'inline' }], {
					padding: 5, afterClose: function () {
						$( '.th' ).remove();
					}
				} );
			}
		...
		$( window ).on( 'load', function () {
			...
			$( '.home-labels .bx-controls-direction>a' ).attr( 'aria-hidden', 'true' ).attr( 'tabindex', '-1' );

			$( '.menu-item-has-children>a' ).each( function ( index ) {
				$( this ).attr( 'aria-label', $( this ).text() + ' Menu' );
			} );
		} );

		$( '#sliderblock' ).on( 'focusin', '.bx-prev', function ( e ) {
			main_slider.stopAuto();
		} );

		$( '#sliderblock' ).on( 'focusout', '.bx-next', function ( e ) {
			main_slider.startAuto();
		} );
		
		$( '.home-logo svg' ).attr( 'role', 'img' ).attr( 'aria-label', 'Sony Music' );
		
		$( '.owl-dot' ).each( function( index, element ) {
		    $( this ).attr( 'aria-label', 'Slide ' + index + 1 );
		});
		
		$( '.sa_slide_link_icon' ).each( function( index, element ) {
		    $( this ).attr( 'aria-label', 'Slide Link ' + index + 1 );
		});		
	}
)( jQuery );
